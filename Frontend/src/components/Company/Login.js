...
class Login extends React.Component {
  ...
  login = async (e) => {
    ...
    if (email !== "" && password !== "") {
      try {
        let data = await this.props.loginCompanyMutation({
          variables: {
            email: email,
            password: password,
          },
        ...
        if (!data.loading) {
          localStorage.setItem("id", data.data.loginCompany._id);
          localStorage.setItem("type", data.data.loginCompany.__typename);
  ...
  render() {
    // if login in then redirect to the company profile
